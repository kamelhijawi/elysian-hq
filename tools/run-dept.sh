...
d=[x for x in m['departments'] if x['id']==sys.argv[1]][0]
...
PROMPT="You are the $DEPT department bot for Elysian. Today is $TODAY. Read CLAUDE.md in this folder, then run the recipe $RECIPE exactly: load the files it names, follow its sequence, write the outputs it names, then run: git add -A && git commit -m \"$DEPT: $(basename $RECIPE .md) $TODAY\". Read-only toward Salesforce and Gmail. Never send anything. Never write outside this folder. If a data source is unreachable, write what you could not get into the output and still commit. Finish with one line: DONE <files written>."
